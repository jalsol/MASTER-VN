...
from master_cross_attn_gate import MASTERCrossAttnGateModel
...
data_dir = Path("data/vn100_10y/vn100_10y")
with open(data_dir / "vn100_10y_dl_train.pkl", "rb") as f:
    dl_train = pickle.load(f)
with open(data_dir / "vn100_10y_dl_valid.pkl", "rb") as f:
    dl_valid = pickle.load(f)
with open(data_dir / "vn100_10y_dl_test.pkl", "rb") as f:
...
for seed in range(5):
    print(f"\n{'=' * 80}")
    print(f"VN100 10Y MASTER Cross-Attn Gate - Seed {seed}")
    print(f"{'=' * 80}")

    model = MASTERCrossAttnGateModel(
        d_feat=158,
        d_model=256,
        t_nhead=4,
        s_nhead=2,
        ...
        save_path="model",
        save_prefix="vn100_10y_cross_attn_gate",
        ffn_expand=1,
        use_amp=False,
        gate_n_heads=2,
        gate_dropout=0.0,
...
print(f"\n{'=' * 80}")
print("VN100 10Y MASTER CROSS-ATTN GATE - FINAL RESULTS (5 seeds)")
print(f"{'=' * 80}")
