...
for seed in range(5):
    print(f"\n{'=' * 80}")
    print(f"VN100 MASTER LGBM-Gate - Seed {seed}")
    print(f"{'=' * 80}")
    ...
    model = MASTERLGBMGateModel(
        ...
        save_path="model",
        save_prefix="vn100_lgbm_gate",
        ffn_expand=1,
        use_amp=False,
        prior_mix=0.45,
        shuffle_mix=0.03,
        dynamic_prior_mix=0.75,
        lgbm_num_leaves=63,
        lgbm_learning_rate=0.03,
        lgbm_n_estimators=500,
        lgbm_feature_fraction=0.85,
        lgbm_target="direction",
    )
...
print(f"\n{'=' * 80}")
print("VN100 MASTER LGBM-GATE - FINAL RESULTS (5 seeds)")
print(f"{'=' * 80}")
