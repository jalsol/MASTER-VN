...
from master_moe_gate import MASTERMoEGateModel
...
for seed in range(5):
    print(f"\n{'=' * 80}")
    print(f"VN100 10Y MASTER MoE-Gate - Seed {seed}")
    print(f"{'=' * 80}")

    model = MASTERMoEGateModel(
        d_feat=158,
        d_model=256,
        t_nhead=4,
        s_nhead=2,
        ...
        save_path="model",
        save_prefix="vn100_10y_moe_gate",
        ffn_expand=1,
        use_amp=False,
    )
...
print(f"\n{'=' * 80}")
print("VN100 10Y MASTER MOE-GATE - FINAL RESULTS (5 seeds)")
print(f"{'=' * 80}")
