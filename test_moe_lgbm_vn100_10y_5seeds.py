...
from master_moe_lgbm import MASTERMoELGBMGateModel
...
for seed in range(5):
    print(f"\n{'=' * 80}")
    print(f"VN100 10Y MASTER MoE+LGBM Gate - Seed {seed}")
    print(f"{'=' * 80}")

    model = MASTERMoELGBMGateModel(
        d_feat=158,
        d_model=256,
        t_nhead=4,
        s_nhead=2,
        ...
        save_path="model",
        save_prefix="vn100_10y_moe_lgbm",
        ffn_expand=1,
        use_amp=False,
        prior_mix=0.2,
    )
...
print(f"\n{'=' * 80}")
print("VN100 10Y MASTER MOE+LGBM GATE - FINAL RESULTS (5 seeds)")
print(f"{'=' * 80}")
